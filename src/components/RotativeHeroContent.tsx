    ...
    >
      <CarouselContent className="h-full"> {/* Garante que o conteúdo do carrossel ocupe a altura total */}
        {slides.map((slide, index) => (
          <CarouselItem key={index} className={`h-full flex flex-col ${slide.backgroundClasses}`}> {/* Aplica o gradiente, garante altura total e torna flex-col */}
            <div className="relative z-10 max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8 h-full flex-grow"> {/* Adiciona flex-grow para preencher o espaço */}
